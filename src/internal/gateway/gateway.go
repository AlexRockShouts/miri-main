...
func New(cfg *config.Config, st *storage.Storage) *Gateway {
	...
	if cfg.Tools.CronEnabled {
		gw.cronMgr = cron.NewCronManager(gw.Storage, gw.DelegatePrompt)
		gw.cronMgr.Start()
	}
...
func (gw *Gateway) DelegatePrompt(sessionID string, prompt string) (string, error) {
...
func (gw *Gateway) ChannelChat(channel, device, prompt string) (string, error) {
	resp, err := gw.DelegatePrompt(device, prompt)
	if err != nil {
		return "", err
	}
