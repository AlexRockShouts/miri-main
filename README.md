...
- **File Management**:
  - `FileManagerTool`: Allows the agent to list and share files from the `.generated` sandbox.
  - `GET /api/v1/files/*filepath`: Direct download access to files generated by the agent (e.g., movies, documents) within the `.generated` sandbox.
...
- **Long-term Memory**: Durable storage using a persistent vector database (`chromem-go`). The **Brain** component automatically maintains memory quality through a continuous compaction loop:
  - **Vector Database**: Uses [chromem-go](https://github.com/philippgille/chromem-go) for high-performance, persistent vector storage.
    - **Persistence**: Data is stored in `~/.miri/vector_db` (or your custom `MIRI_STORAGE_DIR`).
    - **Embeddings**: Supports multiple providers including OpenAI, Mistral, Ollama, Jina, Cohere, and local OpenAI-compatible APIs.
    - **Native Mode**: Can operate entirely offline using a built-in static embedder (`qwen3_embedding_0.6b_pca384`).
  - **Maintenance Tasks**:
    - **Fact Extraction**: Automatically extracts important facts and preferences from every interaction.
    - **Deduplication**: Periodically merges similar facts using LLM-based deduplication.
    - **Summary Consolidation**: Merges related conversation summaries to prevent redundancy.
    - **Cleanup**: Automatically removes low-confidence or old, never-retrieved items.
    - **Promotion**: Identifies important facts embedded in summaries and promotes them to standalone fact entries.
    - **Auto-Trigger**: Maintenance runs every 100 interactions, on session start, on server shutdown, or when **60% of the model's context window** is reached.
